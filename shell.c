...
int move(int nr_params, char params[][1024])
{
  ...
  for(i = 1; i < nr_params; i++)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if(destination_status == -1) // check what user provided as destination, can be a directory, a file with full path or a file from the current directory
        {
          j = strlen(params[i])-1;
          while(j > 0 && params[i][j] != '/')
            j--;
          if(j > 0) // check if a file from the current directoy was given (no '/' before file name)
          {
            strncpy(aux, params[i], j); // copy the directory in an auxiliary variable
            aux[j] = '\0'; // stirng terminator
            destination_status = stat(aux, &st); // stat to see if the directory exists
          }
          else
          {
            destination_status = -3; // new file name from the current directory case
          }
        }
        else
        {
  ...
  {
    if(destination_status != -3 && (st.st_mode & S_IFMT) == S_IFDIR)
    {
...
int copy(int nr_params, char params[][1024])
{
  ...
  {
    if(destination_status != -3 && (recursive == 1) && ((st.st_mode & S_IFMT) == S_IFDIR))
    {
      stat(params[file_name_index], &st); // could have used 2 struct stat variables, too lazy
      if((st.st_mode & S_IFMT) == S_IFDIR)
      {
    ...
    else
    {
      regular_copy:
        ...
        if((st.st_mode & S_IFMT) == S_IFDIR)
        {
          ...
          fd_r = open(params[file_name_index], O_RDONLY);
        printf("params[destination_name_index] = %s\n", params[destination_name_index]);
        fstat(fd_r, &st);
        fd_w = open(params[destination_name_index], O_CREAT | O_WRONLY | O_TRUNC, st.st_mode);
        while(read(fd_r, &c, 1) == 1)
        {
